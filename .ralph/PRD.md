...
Ralph eliminates these by wrapping Ralph loops in a familiar email-like interface backed by auto-scaling AWS infrastructure.
...
const taskDef = new aws.ecs.TaskDefinition("ralph-agent-task", {
    ...
    containerDefinitions: JSON.stringify([
        {
            name: "ralph-agent",
            image: "ghcr.io/ralph/agent:latest",
            essential: true,
            logConfiguration: {
                logDriver: "awslogs",
                options: {
            ...
            secrets: [{ name: "LLM_API_KEY", valueFrom: "{{secrets_arn}}" }],
        },
    ]),
});
...
const scaleDownPolicy = new aws.appautoscaling.Policy("ralph-scale-down", {
    ...
    stepScalingPolicyConfiguration: {
        ...
        stepAdjustments: [
            { scalingAdjustment: 0, metricIntervalUpperBound: 0 },
        ],
...
Ralph agents are compatible as deepagentsjs subagents via a standard protocol.
...
```typescript
import { Ralph } from "@ralph/sdk";

const ralph = new Ralph({
    apiKey: "ralph_abc123...",
    baseUrl: "https://api.ralph.com/api/v1",
});
